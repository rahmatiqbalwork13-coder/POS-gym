...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Header */}
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-card/80 backdrop-blur-xl border-b border-border/50 z-40 flex items-center justify-between px-4">
      ...
      <aside className={cn(
        "flex flex-col w-64 shrink-0 border-r border-border/50 bg-card/80 backdrop-blur-xl h-screen sticky top-0 z-50",
        "lg:translate-x-0 lg:static",
        "fixed left-0 top-0 transition-transform duration-300 ease-in-out",
        isOpen ? "translate-x-0" : "-translate-x-full"
      )}>
        {/* Logo - Desktop only */}
        <div className="hidden lg:flex h-16 items-center gap-3 px-4 border-b border-border/50">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center shadow-lg shadow-primary/25">
            <Dumbbell className="w-5 h-5 text-white" />
          </div>
          <div>
        ...
        {/* Mobile Close Button */}
        <div className="lg:hidden flex items-center justify-end p-4 border-b border-border/50">
          <button
            onClick={closeSidebar}
            className="p-2 rounded-lg hover:bg-muted/50 transition-colors"
          >
